...
//서브메뉴 fadeOut하기
  ...
  $(window).resize(function(){//브라우저 크가 변하면
    s_img=$('.m_slide img').width();//이미지 크기를 다시 설정한다.
  });
...
//자바스크립트 시간객체를 사용하여 매 3초마다 함수 호출하여 실행하기
  ...
  mslide.animate({'margin-left':'0px'},500,function(){
        $('.m_slide ul > li:last-child').insertAfter('.m_slide ul > li:first-child');
        mslide.css('margin-left',-s_img);//원래 위치로 오게
  });
  }
  //좌, 우 버튼 클릭시 해당 방향으로 움직이게 하기
  l_btn=$('.m_slide i.fa-angle-left');
  r_btn=$('.m_slide i.fa-angle-right');

  l_btn.click(function(){moveLeft();});
  r_btn.click(function(){moveRight();});
  ...
  $('#pause1').click(function(){
    if(n==0){
      $('#pause1').removeClass('fa-pause').addClass('fa-play');
      clearInterval(Timer);
      n=1;
    }else{
      $('#pause1').removeClass('fa-play').addClass('fa-pause');
